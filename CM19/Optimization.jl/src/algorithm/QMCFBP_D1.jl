function primal_from_dual(𝔓::QMCFBProblem, μ; ϵ=1e-12, ε=1e-8, max_iter=1000)
    ...
    algorithm = MinQuadratic.MQBPAlgorithmPG1(
        ...
        verbosity=-1,
        max_iter=max_iter,      # TODO: set properly
        ε=ε/√n,             # TODO: set properly
        ϵ₀=ϵ)           # TODO: set properly
...
algorithm = QMCFBPAlgorithmD1SG(
                 ...
                 verbosity=1,
                 max_iter=10000, # not useful
                 ε=1e-6,
                 ϵ=1e-12);

test = get_test(algorithm, m=100, n=200, singular=60, active=30);
test.solver.options.memoranda = Set(["norm∂L′", "L′","i′","L", "L"]);
for i in 1:20
           run!(test)
...
L_lb = test.result.result["L′"]
μ = test.result.result["μ′"]
𝔓 = test.problem
@unpack Q, q, l, u, E, b = 𝔓
x = Optimization.MinCostFlow.primal_from_dual(𝔓, μ)
